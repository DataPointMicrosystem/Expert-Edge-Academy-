...
function StarRating({ rating, size = "sm" }: { rating: number; size?: "sm" | "md" }) {
  ...
  return (
    <span className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((n) => (
        <svg key={n} className={s} viewBox="0 0 16 16"
...
export default function CourseDetail() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F9F8F5]">
        <div className="text-center">
          ...
          <h2 className="font-display font-bold text-2xl text-[#1B1F3B] mb-2">Course not found</h2>
          <Link to="/" className="text-[#F5A623] font-semibold hover:underline">← Back to home</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#F9F8F5]">
      {/* Course hero */}
      <section className="bg-gradient-to-br from-[#1B1F3B] to-[#2a2f50] text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 grid lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2">
            ...
            <p className="text-white/70 text-base mb-5 max-w-2xl leading-relaxed">{course.description}</p>

            <div className="flex flex-wrap items-center gap-4 mb-5">
              <div className="flex items-center gap-2">
                <span className="text-[#F5A623] font-bold">{course.rating}</span>
                <StarRating rating={course.rating} size="md" />
                <span className="text-white/50 text-sm">({course.reviews.toLocaleString()} ratings)</span>
              </div>
              <span className="text-white/50 text-sm">{course.enrolled.toLocaleString()} students</span>
            </div>
            ...
            <div className="sticky top-20 bg-white rounded-2xl shadow-2xl overflow-hidden text-gray-900">
              <div className="relative">
                ...
                <div className="absolute inset-0 flex items-center justify-center bg-black/30">
                  <div className="w-14 h-14 rounded-full bg-white/90 flex items-center justify-center shadow-lg cursor-pointer hover:bg-white transition-colors">
                    <svg className="w-6 h-6 text-[#1B1F3B] ml-1" viewBox="0 0 20 20" fill="currentColor">
              ...
              <div className="p-5">
                <div className="flex items-baseline gap-3 mb-1">
                  <span className="font-display font-black text-3xl text-[#1B1F3B]">${course.price}</span>
                  <span className="text-gray-400 line-through text-base">${course.originalPrice}</span>
                  <span className="text-green-600 font-bold text-sm">{discount}% off</span>
                </div>
                <p className="text-xs text-red-500 font-semibold mb-4">⏳ 2 days left at this price!</p>
                ...
                <p className="text-[10px] text-gray-400 text-center mt-3">30-Day Money-Back Guarantee</p>

                <div className="mt-4 pt-4 border-t border-gray-100">
                  <p className="text-xs font-bold text-gray-700 mb-2">This course includes:</p>
      ...
      <div className="lg:hidden sticky top-16 z-40 bg-white border-b border-gray-200 shadow-sm px-4 py-3 flex items-center justify-between gap-3">
        <div>
          <span className="font-display font-black text-xl text-[#1B1F3B]">${course.price}</span>
          <span className="text-xs text-gray-400 line-through ml-2">${course.originalPrice}</span>
        </div>
        <div className="flex gap-2">
          <button onClick={() => addToCart(course)} disabled={inCart} className={`px-4 py-2 rounded-xl text-xs font-bold border-2 transition-all ${inCart ? "border-green-200 text-green-700" : "border-[#1B1F3B] text-[#1B1F3B] hover:bg-[#1B1F3B] hover:text-white"}`}>
            {inCart ? "✓ In cart" : "Add to cart"}
          </button>
          <button onClick={handleBuyNow} className="px-4 py-2 rounded-xl text-xs font-bold bg-[#F5A623] text-[#1B1F3B] hover:opacity-90">
            Buy now
          </button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 grid lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 flex flex-col gap-10">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            <h2 className="font-display font-bold text-xl text-[#1B1F3B] mb-4">What you'll learn</h2>
              ...
              {course.whatYoullLearn.map((item) => (
                <div key={item} className="flex items-start gap-3">
                  <div className="w-5 h-5 rounded-full bg-[#F5A623]/20 flex items-center justify-center shrink-0 mt-0.5">
                    <svg className="w-3 h-3 text-[#c47a00]" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
                      <path d="M2 6l3 3 5-5" />
                    </svg>
                  </div>
          ...
          {/* Requirements */}
          <div>
            <h2 className="font-display font-bold text-xl text-[#1B1F3B] mb-4">Requirements</h2>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-display font-bold text-xl text-[#1B1F3B]">Course content</h2>
              <span className="text-xs text-gray-400">{course.sections.length} sections · {totalLectures} lectures · {course.hours}h total</span>
            </div>
            <div className="flex flex-col gap-2">
              {course.sections.map((section, si) => (
                <div key={si} className="border border-gray-200 rounded-xl overflow-hidden">
                  ...
                  >
                    <div>
                      <p className="font-semibold text-sm text-[#1B1F3B]">{section.title}</p>
                      <p className="text-xs text-gray-400 mt-0.5">{section.lectures.length} lectures</p>
                    </div>
                    <svg
                  ...
                  {expandedSection === si && (
                    <div className="divide-y divide-gray-100">
                      {section.lectures.map((lecture) => (
                        <div key={lecture.id} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 transition-colors">
                          ...
                          <div className="flex-1">
                            <p className={`text-sm ${lecture.completed ? "text-gray-400 line-through" : "text-gray-700"}`}>
                              {lecture.title}
                            </p>
                          </div>
                          <div className="flex items-center gap-2">
                            {lecture.free && (
                              <span className="text-[10px] font-bold text-[#1B1F3B] bg-[#F5A623]/20 px-2 py-0.5 rounded-full">Preview</span>
                            )}
                            <span className="text-xs text-gray-400 whitespace-nowrap">{lecture.duration}</span>
                          </div>
                        </div>
                      ))}
                    </div>
          ...
          {/* Instructor */}
          <div>
            <h2 className="font-display font-bold text-xl text-[#1B1F3B] mb-5">Your instructor</h2>
            <div className="flex items-start gap-5 bg-white rounded-2xl p-6 border border-gray-100">
              ...
              />
              <div>
                <h3 className="font-display font-bold text-lg text-[#1B1F3B]">{course.instructor}</h3>
                <p className="text-sm text-gray-500 mb-3">{course.category} Expert & Instructor</p>
                <div className="flex flex-wrap gap-4 text-xs text-gray-500 mb-3">
                  <span>⭐ {course.rating} Instructor Rating</span>
                  <span>👥 {(course.enrolled * 1.4).toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ",")} Students</span>
                  <span>🎥 {course.sections.length * 3}+ Courses</span>
                </div>
                <p className="text-sm text-gray-600 leading-relaxed">
                  {course.instructor} is a senior {course.category.toLowerCase()} professional with 10+ years of industry experience.
                  Known for practical, project-based teaching that gets students real results fast.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pb-16">
        <div className="bg-gradient-to-r from-[#1B1F3B] to-[#2d3360] text-white rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-5">
          <div>
            <h3 className="font-display font-bold text-xl mb-1">Ready to start learning?</h3>
            <p className="text-white/60 text-sm">Join {course.enrolled.toLocaleString()} students already enrolled.</p>
          </div>
          <div className="flex gap-3">
            <button onClick={handleBuyNow} className="px-6 py-3 rounded-xl bg-[#F5A623] text-[#1B1F3B] font-bold text-sm hover:opacity-90 transition-opacity shadow-md">
              Enroll now — ${course.price}
            </button>
            <Link to={`/courses/${course.id}/learn`} className="px-6 py-3 rounded-xl border border-white/25 font-bold text-sm hover:bg-white/10 transition-colors">
              Preview for free
            </Link>
          </div>
        </div>
